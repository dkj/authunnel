...
### Option A.1: Let the client authenticate with the IdP (`--login`)

If you do not want to pre-populate `ACCESS_TOKEN`, the client can perform an interactive OAuth2 Authorization Code + PKCE login.
It spins up a temporary local callback webserver (default `http://127.0.0.1:18085/callback`) to receive the IdP redirect.
...
The client prints the login URL in the terminal. After browser login completes, it exchanges the returned code for an access token and proceeds with the tunnel.
...
- OIDC discovery and PKCE helper behavior for interactive login mode.
