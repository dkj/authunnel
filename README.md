...
- `--oidc-issuer`
- `--oidc-client-id`
- `--oidc-metadata-url` to use trusted authorization-server metadata at a URL other than the path derived from `--oidc-issuer`. Changing it discards the token cache and requires a fresh login. See [Issuer metadata and key discovery](docs/DEPLOYMENT.md#issuer-metadata-and-key-discovery)
