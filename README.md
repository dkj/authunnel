...
- **Bounded issuer metadata and JWKS fetches**: both the server-side validator and the managed client use conservative dial, TLS-handshake, response-header, and overall timeouts. Derived and overridden metadata discovery is bounded during startup. `--oidc-jwks-uri` skips discovery so server egress can be restricted to the key endpoint; the first authenticated request and later key rotation still require that endpoint. See [Issuer metadata and key discovery](docs/DEPLOYMENT.md#issuer-metadata-and-key-discovery).
- **No internal-address pivot from discovered configuration**: an address named by a *remote* metadata document — the issuer, the authorization-server metadata URL, or an endpoint it advertises — is refused when it resolves into the built-in protected set (loopback, IPv4/IPv6 link-local including cloud IMDS, unspecified, multicast). Private networks are deliberately not refused, and the guard applies only to values the tunnel server chose. Enforced at three layers. **Behind a forward proxy the two connection-level layers do not apply to an `https` destination** — this client makes no connection of its own, so certificate verification takes over, and that binds the hostname rather than establishing the address is external. The static resolution check still runs, and still refuses a name local DNS resolves into the protected set. The conditions that model rests on, and what `--oidc-issuer` does and does not answer, are in [Discovered addresses and the internal-address guard](docs/DEPLOYMENT.md#discovered-addresses-and-the-internal-address-guard).
- **No transport downgrade on the auth path**: neither side will fetch credentials or signing keys over a weaker transport than the metadata that named them. An `https` metadata document may not advertise plaintext endpoints, and redirects may not move a metadata, JWKS, or token fetch off `https`. Advertised endpoints must be `http(s)` URLs with a host in every mode, including under `--insecure-oidc-issuer` — this is what keeps a scheme like `file://` out of the authorization URL that the client hands to the OS. Between two HTTPS origins a metadata document may be delegated freely, but a token request may not change origin at all: a 307 preserves the body, so that hop would forward a refresh token or an authorization code. A metadata fetch is pinned the same way in the one case where an origin was promised — see [What `--oidc-issuer` pins, precisely](docs/DEPLOYMENT.md#what---oidc-issuer-pins-precisely). On the client, a refusal is terminal rather than falling back to an interactive login that would fail the same way. The PKCE loopback callback stays plaintext by design (RFC 8252). Details in [docs/DEPLOYMENT.md](docs/DEPLOYMENT.md#transport-rules-on-the-auth-path).
...
(`--oidc-client-id`, and either `--oidc-issuer` or `--oidc-metadata-url`, …) if the
server is run with `--no-resource-metadata`, or to override what it publishes.
...
- `--oidc-issuer`
- `--oidc-client-id`
- `--oidc-metadata-url` to use trusted authorization-server metadata at a URL other than the path derived from `--oidc-issuer`. Usable with or without `--oidc-issuer`; without one the document's own issuer is adopted, which trades away the consistency check. Changing it discards the token cache and requires a fresh login. See [Issuer metadata and key discovery](docs/DEPLOYMENT.md#issuer-metadata-and-key-discovery)
