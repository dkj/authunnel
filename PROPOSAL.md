...
- [`aws_cf_authunnel_testdev_kemp_free.yaml`](aws_cf_authunnel_testdev_kemp_free.yaml) — the same topology again, but with a **Progress Kemp LoadMaster in its free edition** (20 Mb/s, perpetual) as the reverse proxy, for parity with the other Sanger-operated platform. The L7 service is applied after boot by `configure_kemp.sh` over the LoadMaster RESTful API (`:8443`). One practical difference from vTM: the free edition has a short manual first-boot rather than user-data config — the SSH setup console (initial login is `bal` / the EC2 instance ID), then accepting the EULA in the WUI. In practice the AMI arrived already licensed (no Kemp ID needed) and with the admin WUI already on `:8443`, leaving `:443` free for the virtual service. So this stack is "boot + a short manual first-boot + REST config", a little less hands-off than the vTM one.

All three are deliberately **test/dev** stacks, not production blueprints: they run the servers with open egress (`--allow-open-egress`), the resolved-IP guard disabled (`--no-ip-block`), and debug logging — the opposite of the locked-down posture proposed above — so they are useful for learning and evaluation but a production deployment would tighten each of those.
...
1. **Familiarise**: stand up one of the included AWS reference deployments (the [generic NLB](aws_cf_authunnel_testdev_generic.yaml), [Ivanti vTM Community Edition](aws_cf_authunnel_testdev_ivanti_vtm_ce.yaml), or [Kemp LoadMaster free edition](aws_cf_authunnel_testdev_kemp_free.yaml) variant) to see the end-to-end topology run in a throwaway test/dev environment before building on Sanger infrastructure.
