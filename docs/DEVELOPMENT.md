...
- protected-resource metadata: the published document at both well-known shapes, hints appearing only when configured, `resource` following `X-Forwarded-*` only in the trusting mode, the wire keys themselves (asserted as JSON, since every other test decodes into the Go type), a declared `--resource-url` published normalised, and the `WWW-Authenticate` matrix — no error code when no credential was sent, `error="invalid_token"` when one failed, nothing on `200` or on an origin refusal
