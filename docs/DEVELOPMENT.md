...
- **Internal addresses named by a remote document are refused at every layer that can see something the others cannot**, and none is redundant: a resolution check on the value (the only protection for the `authorization_endpoint`, since that URL goes to the OS dispatcher rather than through any client of ours), a per-request destination check in the RoundTripper, and a dial-time check that connects to the address it verified. The proxy decision runs *before* the address checks, because it decides whether they describe anything real. `NewBoundedClient` sets `Proxy: http.ProxyFromEnvironment`, and past a proxy the destination is resolved by the proxy, so the address checks report on a lookup the connection will not use. What happens then depends on the scheme, and the distinction is the load-bearing part: **proxied `https` is allowed** because the transport handshakes with the origin itself and validates the certificate against the requested name — a rebound service cannot complete that handshake — while **proxied plaintext is refused**, having nothing to be checked against. The address checks are *skipped* on the proxied path rather than applied, since a proxied network frequently has no local answer for an external name. Two earlier versions of this were wrong in opposite directions: one exempted the proxy hop and documented the residual exposure (a guard that silently does not hold), the other refused every proxied request (withdrawing a feature to close a hole TLS had already closed). A TLS-terminating proxy remains outside all of this by construction, and that is the proxy's egress policy to answer for.
...
Add `--client-id authunnel-cli --client-scopes openid` to publish the client
